...
st.markdown(
    ...
    <style>
      ...
      /* Preview box: keep everything INSIDE */
      .preview-frame {{
        ...
        overflow: hidden;  /* ✅ prevents label from escaping */
      }}

      /* Center download area */
      .dl-row {{
        margin-top: 14px;
        display:flex;
        justify-content:center;
      ...
      /* Make image responsive inside preview frame */
      .preview-frame img {{
        max-width: 100%;
        height: auto;
        display:block;
...
st.markdown('<div class="app-title">🏷️&nbsp; LinkLabel Pro</div>', unsafe_allow_html=True)
...
# ---------------- LEFT: SETTINGS ----------------
with left:
    st.markdown('<div class="card"><h3>Global Settings</h3>', unsafe_allow_html=True)

    label_type = st.selectbox("Label Type", ["Copper", "Fiber 1 Unit", "Fiber 2 Unit"], index=1)
    ...
    st.markdown('<div class="card" style="margin-top:14px;"><h3>Link Configuration</h3>', unsafe_allow_html=True)
    ...
    if label_type == "Copper":
        ...
        default_texts = (
            ["L1", "L2", "L3"] if n_boxes == 3
            else ["2L98.1", "2L98.2", "2L98.3", "2L98.4", "2L100.1", "2L100.2"]
        )
        default_cols = (
            [RED, BLUE, RED] if n_boxes == 3
            else [RED, RED, BLUE, BLUE, RED, RED]
        )
...
# ---------------- RIGHT: PREVIEW ----------------
with right:
    st.markdown('<div class="card"><h3>Label Preview</h3>', unsafe_allow_html=True)

    # ✅ Always render inside the preview box
    st.markdown('<div class="preview-frame">', unsafe_allow_html=True)

    if st.session_state.label_img is None:
        st.markdown(
            "<div style='color:#64748b;font-size:14px;text-align:center;'>Generate a label to preview it here.</div>",
            unsafe_allow_html=True,
        )
    else:
        # Use a slightly smaller display size so it fits nicely
        st.image(st.session_state.label_img, use_container_width=True)

    st.markdown("</div>", unsafe_allow_html=True)

    # Download button centered, inside same card (not outside)
